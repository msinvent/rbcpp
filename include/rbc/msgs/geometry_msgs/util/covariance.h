...
namespace rbc::msgs::geometry_msgs::util
{
...
template <typename T, unsigned int N>
std::ostream &operator<<(std::ostream &os, const rbc::msgs::geometry_msgs::util::Covariance<T, N> &cov)
{
  os << "\nCovariance:";
...
template <typename T, unsigned int N>
std::ostream &
operator<<(std::ostream &os, const std::shared_ptr<rbc::msgs::geometry_msgs::util::Covariance<T, N>> &cov)
{
  return os << *cov.get();
}
